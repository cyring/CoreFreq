...
void PowerInterface(SHM_STRUCT *Shm, PROC_RO *Proc_RO)
{
  ...
  else if (Shm->Proc.Features.Info.Vendor.CRC == CRC_INTEL)
  {
    ...
    for (pw = PWR_DOMAIN(PKG); pw < PWR_DOMAIN(SIZE); pw++)
    {
      if (Proc_RO->PowerThermal.PowerLimit[pw].TimeWindow1 > 0)
      {
  ...
  for (cha = 0; cha < Shm->Uncore.MC[mc].ChannelCount; cha++)
  {
   ...
   for (chip = 0; chip < 4; chip++)
   {
	...
	Shm->Uncore.MC[mc].Channel[cha].Timing.PDM_MODE =
		Proc->Uncore.MC[mc].Channel[cha].AMD17h.SPAZ.PwrDownMode;

	Shm->Uncore.MC[mc].Channel[cha].Timing.PDM_AGGR =
		Proc->Uncore.MC[mc].Channel[cha].AMD17h.SPAZ.AggrPwrDownEn;
