...
void json_end_object(struct json_state *state) {
    ...
    assert(state->nested_state[state->depth] == IN_OBJECT || state->nested_state[state->depth] == IN_OBJECT2);
    state->write(state, "}", 1);
...
void json_end_arr(struct json_state *state) {
    assert(state->depth >= 0);
    assert(state->nested_state[state->depth] == IN_ARRAY || state->nested_state[state->depth] == IN_ARRAY2);
    state->write(state, "]", 1);
    state->nested_state[state->depth--] = DEFAULT;
}
